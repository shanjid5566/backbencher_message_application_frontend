...
//       {/* ── Real List Area (Search Results OR Conversations) ── */}
...
//           <>
//             {isSearching && (
//               <div className="p-4 text-center text-sm text-surface-500 flex justify-center items-center gap-2">
...
//                 <button
//                   onClick={() => handleSearchResultSelect(user.id)}
//                   disabled={isCreatingChat}
//                   className="w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-150 text-left hover:bg-surface-800 disabled:opacity-50"
//                 >
...
//         {/* If NOT searching, show Conversations */}
//         {!searchQuery && (
//           <>
...
//                     {/* Avatar */}
//                     <div className="relative shrink-0">
//                       <img src={partner.image || "https://avatar.iran.liara.run/public"} alt={partner.name} className="w-10 h-10 rounded-full object-cover bg-slate-700" />
//                       {partner.isOnline && (
//                         <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-surface-900 rounded-full"></span>
//                       )}
//                     </div>
...
//                         <p className="text-xs truncate text-surface-500">
//                           {lastMessageBody}
//                         </p>
//                       </div>
//                     </div>
...
export default function ChatSidebar({ activeId, onSelect, socket }: ChatSidebarProps) {
  ...
  // 🆕 Active Now ইউজার ফিল্টার করা (যাদের সাথে চ্যাট আছে এবং যারা অনলাইনে আছে)
  const activeUsers = conversations
    ?.flatMap((c) => c.users)
    ?.filter((u) => u.isOnline)
    // Duplicate ইউজার রিমুভ করা
              ...
              {activeUsers.length === 0 ? (
                ...
                activeUsers.map((user) => (
                  <button
                    key={user.id}
                    onClick={() => {
                      // Active Now থেকে ক্লিক করলে ওই কনভারসেশনে চলে যাবে
                      const conv = conversations?.find(c => c.users.some(u => u.id === user.id));
                      if (conv) onSelect(conv.id, user.id);
                    }}
                    className="flex flex-col items-center gap-1.5 shrink-0 group"
                  >
                    <Avatar user={{...user, avatarUrl: user.image || "https://ui-avatars.com/api/?name=User"} as any} size="lg" showStatus />
                    <span className="text-[11px] text-surface-400 group-hover:text-surface-200 transition-colors max-w-[52px] truncate">
                      {user.name.split(" ")[0]}
                    </span>
                  </button>
            ...
            {!isSearching && searchResults?.map((user: any) => (
              <li key={user.id} className="animate-fade-in-up">
                <button onClick={() => handleSearchResultSelect(user.id)} disabled={isCreatingChat} className="w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all text-left hover:bg-surface-800 disabled:opacity-50">
                  <div className="relative shrink-0">
                    <img src={user.image || `https://ui-avatars.com/api/?name=${user.name}&background=random`} alt={user.name} className="w-10 h-10 rounded-full object-cover bg-slate-700" />
        ...
        {!searchQuery && (
          <>
            ...
            {!isLoading && !isError && conversations?.map((conv, idx) => {
              const isActive = conv.id === activeId;
              const partner = conv.users[0];
              // 🆕 লাস্ট মেসেজ যদি কল লগ হয়, তাহলে সেভাবে দেখাবে
              ...
              return (
                <li key={conv.id} style={{ animationDelay: `${idx * 40}ms` }} className="animate-fade-in-up">
                  <button onClick={() => onSelect(conv.id, partner.id)} className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all text-left group ${isActive ? "bg-brand-600/20 border border-brand-500/30" : "hover:bg-surface-800 border border-transparent"}`}>
                    <div className="relative shrink-0">
                      <img src={partner.image || `https://ui-avatars.com/api/?name=${partner.name}&background=random`} alt={partner.name} className="w-10 h-10 rounded-full object-cover bg-slate-700" />
