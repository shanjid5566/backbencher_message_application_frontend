...
export default function ChatWindow({ conversationId, receiverId, onBack, socket, onStartVideoCall, onStartAudioCall }: ChatWindowProps) {
  ...
  // 🔴 ১. সকেট লিসেনার (একবারই কানেক্ট হবে, হ্যাং করবে না)
  useEffect(() => {
    if (!socket || !conversationId) return;
    ...
    const handleNewMessage = (newMessage: any) => {
      if (newMessage.conversationId === conversationId) {
        ...
        // নতুন মেসেজ সিন করে ব্যাকএন্ডে পাঠানো
  ...
  // 🔴 ২. চ্যাট ওপেন করলেই আগের মেসেজগুলো সিন করার লজিক (ইনফিনিট লুপ ফিক্সড)
  useEffect(() => {
    if (messages && messages.length > 0 && receiverId) {
      ...
      if (unseenMessages.length > 0) {
        api.patch("/messages/seen", { conversationId }).then(() => {
          ...
          // লোকাল ক্যাশ আপডেট করে লুপ বন্ধ করা হলো
          queryClient.setQueryData(["messages", conversationId], (old: any) => {
            if (!old) return old;
            return old.map((msg: any) => msg.sender?.id !== session?.user?.id ? { ...msg, status: "SEEN" } : msg);
          });
  ...
  return (
    <div className="flex flex-col flex-1 w-full h-full bg-surface-950 animate-slide-right md:animate-none">
      <header className="flex items-center gap-3 px-4 py-3 border-b border-surface-700/50 bg-surface-900/80 backdrop-blur-sm flex-shrink-0 z-10">
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => partner && onStartAudioCall?.(partner.name)} className="w-8 h-8 rounded-full hover:bg-surface-700 flex items-center justify-center text-surface-400 hover:text-surface-200">
            <Phone size={17} />
          </button>
          <button onClick={() => partner && onStartVideoCall?.(partner.name)} className="w-8 h-8 rounded-full hover:bg-surface-700 flex items-center justify-center text-surface-400 hover:text-surface-200">
            <Video size={17} />
          </button>
          <button className="w-8 h-8 rounded-full hover:bg-surface-700 flex items-center justify-center text-surface-400 hover:text-surface-200">
            <MoreHorizontal size={17} />
          </button>
        </div>
      </header>
